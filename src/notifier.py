...
def send_email_report(
    ...
    results_section = ""
    if gemini_verdict or groq_verdict:
        results_section += "<h3>Submission Results</h3>"
        if gemini_verdict:
            ...
            <div style='background-color: #f9fafb; padding: 15px; border-left: 5px solid #22c55e; margin-bottom: 15px;'>
                <h4>🤖 Gemini (gemini-2.5-flash)</h4>
                <p><b>Status:</b> {gemini_verdict.get("status_msg", "Failed")}</p>
                {"<p><b>Runtime:</b> " + str(g_runtime) + " (Beats " + str(round(g_runtime_pct, 1)) + "%)</p>" if g_accepted else ""}
                {"<p><b>Memory:</b> " + str(g_mem) + " (Beats " + str(round(g_mem_pct, 1)) + "%)</p>" if g_accepted else ""}
            </div>
            """
        ...
        if groq_verdict:
            ...
            <div style='background-color: #f9fafb; padding: 15px; border-left: 5px solid #06b6d4; margin-bottom: 15px;'>
                <h4>⚡ Groq (qwen/qwen3.6-27b)</h4>
                <p><b>Status:</b> {groq_verdict.get("status_msg", "Failed")}</p>
                {"<p><b>Runtime:</b> " + str(q_runtime) + " (Beats " + str(round(q_runtime_pct, 1)) + "%)</p>" if q_accepted else ""}
                {"<p><b>Memory:</b> " + str(q_mem) + " (Beats " + str(round(q_mem_pct, 1)) + "%)</p>" if q_accepted else ""}
            </div>
            """

    code_section = ""
    if gemini_code or groq_code:
        code_section += "<h3>Submitted Solutions</h3>"
        if gemini_code:
            # Escape HTML characters for code block display
